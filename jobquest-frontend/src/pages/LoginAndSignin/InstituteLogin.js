...
export default function InstituteLogin() {
    ...
    const handleLogin = async () => {
        try {
            const response = await axios.post("http://localhost:8070/api/institute/login", {
                email: email,
                password: password
            });
        ...
        <div className="flex h-screen">
            <div className="w-full sm:w-1/2 lg:w-2/5" style={{ backgroundColor: '#e6e6f5' }}>
                ...
                <img src={Man} alt="logo"
                     className='w-3/4 sm:w-1/3 lg:w-80 mx-auto block xl:mt-44 lg:mt-32' />
            </div>
            <div className="sm:w-1/2 lg:w-3/5 bg-white justify-content-center mt-5">
                <p className=' mt-3 text-3xl head items-center text-center'>Institute Login</p>
